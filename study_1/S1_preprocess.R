...
S1_clean_data <- S1_clean_data %>% filter(used_calculator == 0) #remove cheaters - answer yes to 'used a calc', N = 28
S1_clean_data <- S1_clean_data %>% filter(q16_532 != 9543306) # remove cheaters - answered 'impossible' q16 correctly), N = 7
...
# keep the original prolific PID rather than creating numeric participant ids
...
# fill diff_100_obs values for all rows, this is done because 
...
# rearrange columns
S1_long_data <- S1_long_data %>%
  select(prolific_pid:question_observed, everything())
...
S1_long_data$observed_q_perceived_difficulty_unbounded <- S1_perceived_difficulty$unbounded_perceived_difficulty[match(S1_long_data$question_observed,
                                                                                                                       S1_perceived_difficulty$question)]
S1_long_data$new_q_perceived_difficulty_unbounded <- S1_perceived_difficulty$unbounded_perceived_difficulty[match(S1_long_data$question_evaluated,
                                                                                                                  S1_perceived_difficulty$question)]
S1_long_data$observed_q_perceived_difficulty <- S1_perceived_difficulty$perceived_difficulty[match(S1_long_data$question_observed,
                                                                                                   S1_perceived_difficulty$question)]
S1_long_data$new_q_perceived_difficulty <- S1_perceived_difficulty$perceived_difficulty[match(S1_long_data$question_evaluated,
                                                                                              S1_perceived_difficulty$question)]
...
# 
# S1_long_data$observed_q_objective_difficulty_unbounded <- S1_objective_difficulty$unbounded_objective_difficulty[match(S1_long_data$question_observed,
#                                                                                                                        S1_objective_difficulty$question)]
# S1_long_data$new_q_objective_difficulty_unbounded <- S1_objective_difficulty$unbounded_objective_difficulty[match(S1_long_data$question_evaluated,
#                                                                                                                   S1_objective_difficulty$question)]
# S1_long_data$observed_q_objective_difficulty <- S1_objective_difficulty$objective_difficulty[match(S1_long_data$question_observed,
#                                                                                                    S1_objective_difficulty$question)]
# S1_long_data$new_q_objective_difficulty <- S1_objective_difficulty$objective_difficulty[match(S1_long_data$question_evaluated,
#                                                                                               S1_objective_difficulty$question)]
...
# select columns and rename question fields for exclusions
S1_excluded_participants <- S1_excluded_participants %>% select(!(q1_537:q16_552) &
                                                                  !(start_date:free_consent))
S1_excluded_participants <- S1_excluded_participants %>% select(
  x1_judgement_matrix_x1:x30_difficulty_1,
  q1_517:used_calculator,
  sc0:time_on_question16
...
# keep the original prolific PID for excluded participants as well
...
count_excluded <- S1_excluded_participants_long %>%
  summarise(count_excluded <- length(unique(
    S1_excluded_participants_long$prolific_pid
...
write.csv(
  ...
  here::here("study_1", "data", "clean", "S1_perceived_difficulty.csv"),
  row.names = FALSE
)

write.csv(
  S1_objective_difficulty,
  here("study_1", "data", "clean", "S1_objective_difficulty.csv"),
  row.names = FALSE
)
...
write.csv(
  long_data_with_timing,
  here("study_1", "data", "clean", "long_data_with_timing.csv"),
  row.names = FALSE
)

write.csv(
  S1_excluded_participants_long,
  here(
    "study_1",
    "data",
    "clean",
    "S1_excluded_participants_long.csv"
  ),
  row.names = FALSE
)
